...
class Hero(DungeonCharacter, ABC):
    ...
    def __str__(self):
        ...
        if isinstance(self, Warrior):
            hero_info += "The Warrior "
        # elif isinstance(self, Thief):
        #     monster_info += "The Thief "
        # elif isinstance(self, Priestess):
        #     monster_info += "The Priestess "

        hero_info += f"{self._name}\n"
        hero_info += f"Hit Points: {self.hit_points}\n"
        hero_info += f"Attack Speed: {self._attack_speed}\n"
        hero_info += f"Minimum Damage: {self._min_damage}\n"
        hero_info += f"Maximum Damage: {self._max_damage}\n"
        hero_info += f"Heal Points: {self._h_potion_ct}\n"
        hero_info += f"Heal Points: {self._v_potion_ct}\n"
        hero_info += f"Heal Points: {self._pillar}\n"
        hero_info += f"Heal Points: {self._chance_to_block}\n"
        hero_info += f"Heal Points: {self._player_name}\n"
        # if self.has_fainted:
