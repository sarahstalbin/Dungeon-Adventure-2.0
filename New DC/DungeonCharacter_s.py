...
class DungeonCharacter(ABC):
    ...
    def attack(self, opponent):
        """ Attacks and causes damage to an opponent """
        print(f"{self.name} is attacking {opponent.name}")
        print(f"{self.name} hit points is {self.hit_points} and the {opponent.name} is {opponent.hit_points}")
        print(self)
        print(opponent)
        if self.can_attack():
            damage = self.get_damage()
            opponent_hit_points = opponent.hit_points - damage
            opponent.hit_points = opponent_hit_points

            print(f"The {self.name} attacks {opponent.name} for {damage} damage points")
            print(f"The {self.name} hit points is {self.hit_points} and the {opponent.name} is {opponent.hit_points}")
            return True
        else:
            print(f" {self.name} couldn't attack {opponent.name}  ")
            print(f"The {self.name} hit points is {self.hit_points} and the {opponent.name} is {opponent.hit_points}")
            return False
